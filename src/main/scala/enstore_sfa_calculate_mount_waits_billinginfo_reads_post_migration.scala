...
object EnstoreSFACalculateMountWaitsBillinginfoReadsPostMigration {
    ...
    partition.foreach { row: org.apache.spark.sql.Row =>
      if (1 == 1) {
        if (!row.isNullAt(1) && !row.isNullAt(4) && !row.isNullAt(14)) {
          ...
          val volume = row.getString(14) // migration volume
  
          val mount_query = s"select start, tapemount_id from tape_mounts"
          ...
          if (rs2.next()) { // Found a mount for the volume, good!
            ...
            rs2.close()
            val mod_query = s"update small_file_billinginfo_reads set migration_next_mount_id = $mount_id, migration_mount_wait = AGE('$start', '$datestamp') where sfbr_id = $sfbr_id"

...
object EnstoreSFACalculateMountWaitsBillinginfoReads {
      ...
      if (1 == 1) {
        if (!row.isNullAt(1) && !row.isNullAt(4) && !row.isNullAt(5)) {
          ...
          if (rs2.next()) { // Found a mount for the volume, good!
            ...
            rs2.close()
            val mod_query = s"update small_file_billinginfo_reads set next_mount_id = $mount_id, mount_wait = AGE('$start', '$datestamp') where sfbr_id = $sfbr_id"

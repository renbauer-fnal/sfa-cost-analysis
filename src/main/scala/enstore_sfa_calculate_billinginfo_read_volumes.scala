...
object EnstoreSFACalculateBillinginfoReadVolumes {
  ...
  private def runEnstoreSFAAnalysis(spark: SparkSession, numPartitions: Int): Unit = {
    ...
    partition.foreach { row: org.apache.spark.sql.Row =>
      if (!row.isNullAt(0)) {
        val bfid = row.getString(0)
        if (bfid.substring(0, 4) == "CDMS" && !row.isNullAt(3) && !row.isNullAt(4)) {
          val epoch = bfid.substring(4,14).toInt
          val file_family = row.getString(3)
          ...
          if (rs2.next()) { // Found a mount for the file family, good!
            ...
            rs2.close()
            val mod_query = s"update small_file_billinginfo_reads set volume = '$volume' where sfbr_id = $sfbr_id"
